...
class Agent(object):
    ...
    def build_actor_critic_network(self):
        ...
        policy_discrete_probs = Dense(self.env.discrete_action_space.n, activation = 'softmax', name = "discrete_action_output")(dense3)
        policy_continuous = Dense(self.env.continuous_action_space.shape[0], activation = 'sigmoid', name = "policy_output")(dense3)
        policy_continuous = Lambda(lambda i: i*self.act_range+self.act_min, name = "policy_output_actual_value")(policy_continuous)
        
        Values = Dense(1, activation = 'linear', name = "Critic_output")(dense3) # Critic outputs a single value 
