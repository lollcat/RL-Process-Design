...
class Simulator:
    ...
    def condensor_pressure_calculator(self, tops):
        condensor_temperature = 60 + 273 # K - so CW can be used
        vapour_pressures = self.vapour_pressure_calculator(condensor_temperature)
        p_bub = np.multiply(vapour_pressures, tops/sum(tops))
        return p_bub

    def tops_temperature_calculator(self, tops, p_bub):
        tops_composition = tops/sum(tops)
        temp_guess = np.array([80 + 273])
        dew_func = lambda temperature: tops_composition/self.Kvalues_calculator(temperature)
    ...
    def Kvalues_calculator(self, temperature):
