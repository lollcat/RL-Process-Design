...
class Agent(object):
    ...
    def learn(self, state, action, reward, next_state, done, batch_size, verbose = 0):
        ...
        self.actor.fit([state, delta], [actions_discrete, action_continuous], verbose = verbose, batch_size = batch_size) #verbose = 0 stops outputs from displaying
        self.critic.fit(state, target, verbose = verbose, batch_size = batch_size)
